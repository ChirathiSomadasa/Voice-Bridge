...
class WordProgressActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 2. SAVE TO FIREBASE
        if (currentUser != null) {
            saveAndCalculateWordProgress(currentUser.uid, batchIndex, score)
        }
        ...
        llTryAgain.setOnClickListener {
            if (currentUser != null) {
                ...
                prefs.edit().putInt("SAVED_BATCH_LEVEL_2_${currentUser.uid}", batchIndex).apply()

                val updateMap = hashMapOf("level2_batch" to batchIndex)
        ...
        llContinue.setOnClickListener {
            ...
            intent.putExtra("LEVEL_TYPE", 2) // 2 for Words
            startActivity(intent)
            finish()
        }
    }

    private fun saveAndCalculateWordProgress(userId: String, batchIndex: Int, score: Int) {
        val batchKey = "batch_$batchIndex"
        val batchData = hashMapOf(batchKey to score)

        // 1. Save Batch Score to 'student_speech_progress'
        val scoresRef = db.collection("student_speech_progress")
            .document(userId)
            .collection("level_2_words")
            .document("batch_scores")

        scoresRef.set(batchData, SetOptions.merge())
            .addOnSuccessListener {
                // 2. Read back all scores to calculate average
                scoresRef.get().addOnSuccessListener { document ->
                    if (document.exists() && document.data != null) {
                        ...
                        if (batchCount > 0) {
                            ...
                            val summaryData = hashMapOf("overall_word_progress" to overallAverage)

                            // 3. Save Overall Average to 'student_speech_progress' (MATCHING LETTERS)
